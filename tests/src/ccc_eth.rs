//! Tests for ccc-eth-delegate script with delegate lock integration.
//!
//! These tests verify the ETH CCC lock script works correctly when invoked
...
// =============================================================================
// ETH-specific crypto helpers (reproduce on-chain logic from ccc-eth/src/entry.rs)
// =============================================================================
...
/// Reproduces the on-chain `message_hash` from ccc-eth/src/entry.rs.
fn eth_message_hash(sighash_hex: &str) -> [u8; 32] {
    ...
    let content_len = (CKB_PREFIX.len() + sighash_hex.len() + CKB_SUFFIX.len()) as u8;
    data.extend(format!("{}", content_len).as_bytes());
...
impl EthKeyPair {
    fn new() -> Self {
        ...
        let pubkey_hash = keccak160(&uncompressed.as_bytes()[1..]);
        Self {
            signing_key,
            pubkey_hash,
        }
...
// =============================================================================
// ETH-specific transaction signing
// =============================================================================
...
fn sign_tx_eth(
    ...
    let digest = eth_message_hash(&sighash_hex);
    ...
    // ETH format: bytes[0..64] = r || s, byte[64] = rec_id + 27
...
struct TestContext {
    ...
    ccc_eth_out_point: OutPoint,
    ccc_eth_code_hash: [u8; 32],
}

impl TestContext {
    fn new() -> Self {
        let mut base = DelegateTestBase::new();
        let (ccc_eth_out_point, ccc_eth_code_hash) = base.deploy_binary("ccc-eth-delegate");
        Self {
            base,
            ccc_eth_out_point,
            ccc_eth_code_hash,
    ...
    fn script_cell_deps(&self) -> Vec<CellDep> {
        vec![cell_dep(self.ccc_eth_out_point.clone())]
    }
}
...
#[test]
fn test_ccc_eth_unlock_success() {
    let mut ctx = TestContext::new();
    let owner = EthKeyPair::new();
    let type_id = ctx.base.setup_type_id();
    let code_hash = ctx.ccc_eth_code_hash;
    ...
    let tx = sign_tx_eth(tx, &owner);
    let cycles = verify_and_dump_failed_tx(&ctx.base.context, &tx, 100_000_000)
        .expect("unlock should succeed");
    println!("CCC-ETH delegate lock unlock success cycles: {}", cycles);
}

#[test]
fn test_ccc_eth_multiple_cells() {
    let mut ctx = TestContext::new();
    let owner = EthKeyPair::new();
    let type_id = ctx.base.setup_type_id();
    let code_hash = ctx.ccc_eth_code_hash;
    ...
    println!("CCC-ETH multiple cells success cycles: {}", cycles);
}

#[test]
fn test_ccc_eth_wrong_signature() {
    let mut ctx = TestContext::new();
    let owner = EthKeyPair::new();
    let attacker = EthKeyPair::new();
    let type_id = ctx.base.setup_type_id();
    let code_hash = ctx.ccc_eth_code_hash;
    let type_id_cell = ctx
        .base
    ...
    let tx = ctx
        .base
        .build_unlock_tx(vec![locked_cell], 990, &type_id, deps);
    let tx = sign_tx_eth(tx, &attacker);
...
#[test]
fn test_ccc_eth_missing_type_id_cell() {
    let mut ctx = TestContext::new();
    let owner = EthKeyPair::new();
    let type_id = ctx.base.setup_type_id();
    let locked_cell = ctx.base.create_delegate_locked_cell(&type_id, 1000);
...
#[test]
fn test_ccc_eth_corrupted_witness() {
    let mut ctx = TestContext::new();
    let owner = EthKeyPair::new();
    let type_id = ctx.base.setup_type_id();
    let code_hash = ctx.ccc_eth_code_hash;
